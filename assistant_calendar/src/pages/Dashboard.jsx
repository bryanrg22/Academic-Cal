import { useState, useEffect } from 'react';
import { format } from 'date-fns';
import { useBriefing } from '../hooks/useBriefing';
import { isUrgent } from '../lib/dateUtils';
import { LoadingState, EmptyState } from '../components/LoadingState';
import { DailySummary } from '../components/DailySummary';
import { Summary } from '../components/Summary';
import { UrgentBanner } from '../components/UrgentBanner';
import { ActionItems } from '../components/ActionItems';
import { AssignmentList } from '../components/AssignmentCard';
import { AnnouncementList } from '../components/AnnouncementList';
import { EdPostList } from '../components/EdPostList';
import { GradescopeStatus } from '../components/GradescopeStatus';
import { WeeklyCalendar } from '../components/WeeklyCalendar';
import { QuickAddTask, getPersonalTasks } from '../components/QuickAddTask';

function SectionHeader({ title, icon, count }) {
  return (
    <div className="flex items-center justify-between mb-4">
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 rounded-lg bg-slate-800 border border-slate-700/50 flex items-center justify-center text-slate-400">
          {icon}
        </div>
        <h2 className="font-[family-name:var(--font-display)] text-lg text-slate-100 font-medium">
          {title}
        </h2>
      </div>
      {count !== undefined && (
        <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-slate-800 text-slate-400 border border-slate-700/50">
          {count}
        </span>
      )}
    </div>
  );
}

function Section({ children, className = '' }) {
  return (
    <section className={`mb-10 ${className}`}>
      {children}
    </section>
  );
}

export function Dashboard() {
  const { briefing, loading, error } = useBriefing();
  const [personalTasks, setPersonalTasks] = useState([]);

  // Load personal tasks on mount
  useEffect(() => {
    setPersonalTasks(getPersonalTasks());
  }, []);

  // Mark items as seen after 5 seconds of viewing
  useEffect(() => {
    if (!briefing?.date || !briefing?.newItemKeys?.length) return;

    const timer = setTimeout(async () => {
      try {
        await fetch('https://us-central1-academic-calendar-e63b5.cloudfunctions.net/markAsSeen', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ date: briefing.date })
        });
      } catch (err) {
        console.error('Failed to mark as seen:', err);
      }
    }, 5000);

    return () => clearTimeout(timer);
  }, [briefing?.date, briefing?.newItemKeys?.length]);

  const handleTaskAdded = () => {
    setPersonalTasks(getPersonalTasks());
  };

  if (loading) {
    return <LoadingState />;
  }

  if (error) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 flex items-center justify-center">
            <svg className="w-8 h-8 text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h2 className="text-xl text-slate-200 font-medium mb-2">Unable to load briefing</h2>
          <p className="text-slate-500 text-sm mb-4">
            There was an error connecting to the database. Please check your configuration.
          </p>
          <code className="block text-xs text-rose-400 bg-slate-900 rounded-lg p-3 overflow-auto">
            {error.message}
          </code>
        </div>
      </div>
    );
  }

  if (!briefing) {
    return (
      <div className="min-h-screen bg-slate-950">
        <Header />
        <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <EmptyState />
        </main>
      </div>
    );
  }

  const {
    summary = null,
    actionItems = [],
    assignments = [],
    announcements = [],
    edPosts = [],
    gradescope = [],
    newItemKeys = []
  } = briefing;

  // Combine briefing action items with personal tasks
  const allActionItems = [...actionItems, ...personalTasks];

  // Filter out urgent items from action items (they're shown in the Urgent Banner)
  const nonUrgentActionItems = allActionItems.filter(item => !isUrgent(item.dueDate, item.status));

  // Check if there are any urgent items to show the banner
  const hasUrgentItems = allActionItems.some(item => isUrgent(item.dueDate, item.status)) ||
    assignments.some(a => isUrgent(a.dueDate, a.status));

  return (
    <div className="min-h-screen bg-slate-950">
      {/* Subtle gradient background */}
      <div className="fixed inset-0 bg-gradient-to-br from-slate-950 via-slate-900/50 to-slate-950 pointer-events-none"></div>
      <div className="fixed top-0 left-1/4 w-96 h-96 bg-amber-500/5 rounded-full blur-3xl pointer-events-none"></div>
      <div className="fixed bottom-0 right-1/4 w-96 h-96 bg-blue-500/5 rounded-full blur-3xl pointer-events-none"></div>

      <div className="relative">
        <Header date={briefing.date} />

        <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* Daily Summary - AI Generated */}
          <DailySummary summary={summary} />

          {/* Stats Overview */}
          <Section>
            <Summary
              assignments={assignments}
              actionItems={allActionItems}
              gradescope={gradescope}
            />
          </Section>

          {/* Weekly Calendar View */}
          <Section>
            <SectionHeader
              title="Week View"
              icon={
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
              }
            />
            <WeeklyCalendar assignments={assignments} actionItems={allActionItems} gradescope={gradescope} />
          </Section>

          {/* Urgent Banner - only show if there are urgent items */}
          {hasUrgentItems && (
            <Section>
              <UrgentBanner assignments={assignments} actionItems={allActionItems} />
            </Section>
          )}

          {/* Upcoming Tasks - always show for quick-add */}
          <Section>
            <SectionHeader
              title="Upcoming Tasks"
              count={nonUrgentActionItems.length}
              icon={
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                </svg>
              }
            />
            {nonUrgentActionItems.length > 0 && (
              <ActionItems items={nonUrgentActionItems} onTaskDeleted={handleTaskAdded} newItemKeys={newItemKeys} />
            )}
            <div className={nonUrgentActionItems.length > 0 ? 'mt-4' : ''}>
              <QuickAddTask onTaskAdded={handleTaskAdded} />
            </div>
          </Section>

          {/* Due This Week */}
          {assignments.length > 0 && (
            <Section>
              <SectionHeader
                title="Due This Week"
                count={assignments.length}
                icon={
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                }
              />
              <AssignmentList assignments={assignments} newItemKeys={newItemKeys} />
            </Section>
          )}

          {/* Two column layout for announcements and ed posts */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
            {/* Announcements */}
            {announcements.length > 0 && (
              <Section className="mb-0">
                <SectionHeader
                  title="Announcements"
                  count={announcements.length}
                  icon={
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z" />
                    </svg>
                  }
                />
                <AnnouncementList announcements={announcements} />
              </Section>
            )}

            {/* Ed Discussion */}
            {edPosts.length > 0 && (
              <Section className="mb-0">
                <SectionHeader
                  title="Ed Discussion"
                  count={edPosts.length}
                  icon={
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z" />
                    </svg>
                  }
                />
                <EdPostList posts={edPosts} />
              </Section>
            )}
          </div>

          {/* Gradescope Status */}
          {gradescope.length > 0 && (
            <Section>
              <SectionHeader
                title="Gradescope"
                count={gradescope.length}
                icon={
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                  </svg>
                }
              />
              <GradescopeStatus items={gradescope} />
            </Section>
          )}
        </main>
      </div>
    </div>
  );
}

function Header({ date }) {
  const now = new Date();
  const hour = now.getHours();

  let greeting = 'Good evening';
  if (hour < 12) greeting = 'Good morning';
  else if (hour < 17) greeting = 'Good afternoon';

  // Append T12:00:00 to date string to parse as local noon, avoiding timezone shift issues
  const displayDate = date ? format(new Date(date + 'T12:00:00'), 'EEEE, MMMM d, yyyy') : format(now, 'EEEE, MMMM d, yyyy');

  return (
    <header className="border-b border-slate-800/50 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center justify-between">
          <div className="animate-fade-in-up">
            <h1 className="font-[family-name:var(--font-display)] text-2xl sm:text-3xl text-slate-100 font-medium tracking-tight">
              {greeting}
            </h1>
            <p className="text-slate-500 mt-1 flex items-center gap-2">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              {displayDate}
            </p>
          </div>

          <nav className="flex items-center gap-2 animate-fade-in-up stagger-2">
            <a
              href="/"
              className="px-3 py-2 text-sm font-medium text-amber-400 bg-amber-500/10 rounded-lg border border-amber-500/30 hover:bg-amber-500/20 transition-colors"
            >
              Today
            </a>
            <a
              href="/history"
              className="px-3 py-2 text-sm font-medium text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-colors"
            >
              History
            </a>
            <a
              href="/submit"
              className="px-3 py-2 text-sm font-medium text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-colors flex items-center gap-1.5"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4v16m8-8H4" />
              </svg>
              Submit
            </a>
          </nav>
        </div>
      </div>
    </header>
  );
}
